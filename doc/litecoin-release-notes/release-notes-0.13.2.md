mantra Core version 0.13.2 is now available from:

  <https://download.faithco.in/mantra-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/mantra-project/mantra/issues>
...
updates will be released anymore. Without security updates, using a mantra
...
ECDSA signatures inside mantra transactions now use validation using
...
Previous versions of mantra Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of mantra Core was to raise the default minimum
relay fee.

mantra Core 0.13.2 will have a strict maximum size on the mempool. The
...
mantra Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running mantrad without having to do any manual configuration.
...
mantra Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. mantra Core will relay transactions with
...
entirely in mantra Core 0.13, and it is at this time undecided whether the
...
mantra Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
mantra Core automatically creates a hidden service to listen on, without
manual configuration. mantra Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if mantra Core is listening, and
...
mantrad can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals mantra
Core to use floating fees. By default, mantra Core will use floating
fees.
...
(default: `0.0002` MTR/kB).

At all times, mantra Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MTR.
Furthermore, mantra Core will never create transactions paying less than
...
`mantra.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /mantrarpc>
        ...
        # optional bypass mantrad rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< mantrarpc:<password>
    </Location>
...
Restarting mantrad will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in mantra.conf
...
For example:
    ...
    $ src/mantra-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The mantra Core code base has
...
- `mantra-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `mantra-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the mantra scripting system that in combination with
...
mantra transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of mantra Core.
...
The two phases are distinct in the mantra-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming mantra wallet
  software more difficult and which seriously complicates the design of smart
  contracts for mantra.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each mantra block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of mantras they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of mantras being spent is exactly
...
- **Increased security for multisig:** mantra addresses (both P2PKH addresses
  ...
  believed to be provided by mantra's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of mantra's current ledger (called the UTXO set),
  ...
  note that mantra Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  mantra users to individually opt-in to almost any change in the mantra
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and mantra Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the mantra
client to mantra Core 0.13.2 to signal for segwit support. When a
...
Since mantra Core 0.10.0, nodes have defaulted to only relaying and
...
- `mantra-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `mantra-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
